...
import { Cctv, BellRing, ShieldCheck } from "lucide-react";

export default function QuickHighlights() {
  const items = [
    {
      title: "Premium Indoor Cameras",
      icon: <Cctv size={32} />,
    },
    {
      ...
      <div className="max-w-6xl mx-auto">
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {items.map((item, i) => (
            <div
              key={i}
              className="group relative bg-white border border-slate-100 rounded-3xl p-8 shadow-md hover:shadow-2xl transition-all duration-500 flex flex-col items-center text-center pt-16"
            >
              {/* Round Icon Container - Half Outside + Half Inside */}
